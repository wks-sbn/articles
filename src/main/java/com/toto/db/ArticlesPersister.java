...
public class ArticlesPersister {
    ...
    public Uni<List<Article>> list() {
        return getCollection().find()
                .map(doc -> new Article(
                        ...
                        doc.getDate(PUBLICATION_DATE_KEY).toInstant().atZone(java.time.ZoneId.systemDefault())
                                .toLocalDateTime()))
                .collect().asList();
    }
    ...
    public Uni<Void> save(Article article) {
        Document doc = new Document()
                ...
                .append(PUBLICATION_DATE_KEY, java.util.Date
                        .from(article.publicationDate().atZone(java.time.ZoneId.systemDefault()).toInstant()));
